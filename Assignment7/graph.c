...
int DFS(Graph* g, Vertex* source, Vertex* destination)
{
    ...
    int i;
	struct cirListDeque *searchStack = malloc(sizeof(struct cirListDeque)); // Create a stack for vertices.
	initCirListDeque(searchStack);

	clearVisited(g);

	addFrontCirListDeque(searchStack, source);

    while(!isEmptyCirListDeque(searchStack))
    {
        Vertex *DFSCurrentVertex = backCirListDeque(searchStack);
        removeBackCirListDeque(searchStack);

        if(DFSCurrentVertex == destination)
        {
            return 1;
        }

        for(i = 0; i < DFSCurrentVertex->numNeighbors; i++)
        {
            if(DFSCurrentVertex->neighbors[i]->isVisited == 0)
            {
                addBackCirListDeque(searchStack, DFSCurrentVertex->neighbors[i]);
                DFSCurrentVertex->neighbors[i]->isVisited = 1;
            }
        }

        DFSCurrentVertex->isVisited = 1;
    }
...
int BFS(Graph* g, Vertex* source, Vertex* destination)
{
    ...
    int i;
    struct cirListDeque *searchQueue = malloc(sizeof(struct cirListDeque));
    ...
    while(!isEmptyCirListDeque(searchQueue))
    {
        ...
        Vertex* BFSCurrentVertex = frontCirListDeque(searchQueue);
        removeFrontCirListDeque(searchQueue);

        if(BFSCurrentVertex == destination)
        {
            return 1;
        }

        for(i = 0; i < BFSCurrentVertex->numNeighbors; i++)
        {
            if(BFSCurrentVertex->neighbors[i]->isVisited ==0)
            {
                addBackCirListDeque(searchQueue, BFSCurrentVertex->neighbors[i]);
                BFSCurrentVertex->neighbors[i]->isVisited = 1;
            }

                BFSCurrentVertex->isVisited = 1;
            }
        }
        return 0;
    }
